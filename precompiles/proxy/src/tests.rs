...
fn fails_if_called_by_smart_contract() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let _ = pallet_evm::Pallet::<Runtime>::create_account(H160::from(Alice), vec![10u8], None);
...
fn proxy_proxy_should_fail_if_called_by_smart_contract_for_a_non_eoa_account() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let _ = pallet_evm::Pallet::<Runtime>::create_account(H160::from(Alice), vec![10u8], None);
			let _ = pallet_evm::Pallet::<Runtime>::create_account(H160::from(Bob), vec![10u8], None);
